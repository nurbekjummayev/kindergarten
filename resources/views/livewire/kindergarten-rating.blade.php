<div class="space-y-4">
    ...
    <div class="flex items-center gap-4 pb-3 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center gap-1">
            @for ($i = 1; $i <= 5; $i++)
                <svg class="h-6 w-6 {{ $averageRating >= $i ? 'text-yellow-400' : ($averageRating >= $i - 0.5 ? 'text-yellow-400' : 'text-gray-300') }} fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    @if($averageRating >= $i)
                        <!-- Full star -->
    ...
    <div>
        @auth
            <div class="space-y-2">
                <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">Sizning bahoingiz:</p>
        ...
        @else
            <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                <p class="text-sm text-gray-700 dark:text-gray-300">
                    Baholash uchun <a href="{{ route('filament.client.auth.login') }}" class="text-blue-600 hover:underline dark:text-blue-400 font-semibold">tizimga kiring</a>
                </p>
            </div>
        @endauth
    </div>
